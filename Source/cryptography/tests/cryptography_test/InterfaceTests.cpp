...
/*
 *This step will not be executed for SecApi implementation only, as in the SecApi implementation
...
TEST(Hash, HMAC)
{
    ...
    if (keyId != 0) {
        ...
        if (hashImpl != nullptr) {
            ...
            DumpBuffer(output, sizeof(hash_sha256));
/*
 *This step will not be executed for SecApi implementation only, as in SecApi implementation
...
#ifdef OpenSSL
	    CALL(Vault, SetGet); // Will not work with Sage
#else
	    CALL(Vault, JustSetGet); // For backends other than OpenSSL
#endif
